...
def test(i, face, x, key, stu, ts):
    ...
    while key_index < len(key):
        ...
        while True:
            ...
            i, showID, now, len(key) - key_index, k.strip(), get_iface_status(code)))
            stu.flush()
        ...
        if code == const.IFACE_DISCONNECTED and now < 1:
            stu.write("\r%-6s| %-18s| %5.2fs | %-6s %-15s | %-12s" % (
            i, showID, now, len(key) - key_index, k.strip(), 'BUSY!'))
            stu.flush()
            time.sleep(10)
            continue
        # try next key:
...
def auto_test(keys, timeout, result_file):
    ...
    "-" * 72, "WIFINO", "SSID OR BSSID", "TIME", "KEYNUM", "KEY", "STATUS", "=" * 72)
